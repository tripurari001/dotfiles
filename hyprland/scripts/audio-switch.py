...
def parse_wpctl_status():
    ...
    # strip the * from the default sink and instead append "- Default" to the end. Looks neater in the yofi list this way.
    for index, sink in enumerate(sinks):
        if sink.startswith("*"):
...
# get the list of sinks ready to put into yofi - highlight the current default sink
output = ''
...
# Call yofi and show the list. take the selected sink name and set it as the default sink
selected_sink_name = subprocess.check_output(f"echo '{output}' | yofi dialog", shell=True, encoding='utf-8').strip()
sinks = parse_wpctl_status()
